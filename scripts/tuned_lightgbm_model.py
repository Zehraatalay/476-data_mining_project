...
def build_lightgbm_model(params, random_state=42):
...
def get_param_grid():
    """
    Çok büyük grid yapmıyoruz; yoksa süre patlar.
    Buradaki kombinasyonlar iyi başlangıç verir.
    """
    grid = {
    ...
    for combo in itertools.product(*values):
        ...
        # ufak mantık kısıtı: max_depth çok küçük değilse num_leaves çok abartılı olmasın
        if params["max_depth"] != -1:
            max_reasonable_leaves = 2 ** params["max_depth"]
            if params["num_leaves"] > max_reasonable_leaves:
                continue
...
def tune_lightgbm_on_validation(
    train_df_model,
    val_df_model,
    top_k_features,
    output_dir,
    ...
    for i, params in enumerate(param_grid, start=1):
        ...
        model = build_lightgbm_model(params, random_state=random_state)
        model.fit(
...
def evaluate_best_model_on_test(
    ...
    model = build_lightgbm_model(best_params, random_state=random_state)
    ...
    print_metrics(test_metrics, model_name="Best LightGBM Test Results")
...
def run_single_experiment(
    ...
    # 1) validation tuning
    tuning_result = tune_lightgbm_on_validation(
        sampled_train_model,
        sampled_val_model,
...
def main():
    print("\n" + "=" * 80)
    print("LIGHTGBM TUNED MODEL TRAINING")
    print("=" * 80, flush=True)
    ...
    print("\n" + "=" * 80)
    print("LIGHTGBM TUNED TAMAMLANDI")
    print("=" * 80)
